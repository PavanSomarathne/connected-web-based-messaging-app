...
export default function ChatDisplay({ chat, messages }) {
  ...
  return (
    <Container>
      <Header>
        {recipient ? (
          <Avatar style={{ marginLeft: "15px" }} src={recipient?.photoURL} />
        ) : (
          <Avatar style={{ marginLeft: "15px" }}>{recipiantEmail[0]}</Avatar>
        )}
        ...
        <HeaderInfo>
          {recipient?.name ? (
            ...
            <p>
              Not Registered
              <br />
              <EmailSpan>{recipiantEmail}</EmailSpan>
            </p>
          )}
        </HeaderInfo>
        <HeaderIcons>
      ...
      <InputContainer>
        ...
        <button hidden disabled={!input} type="submit" onClick={sendMsg}>
          Send message
        </button>
        <MicIcon />
      </InputContainer>
...
const EmailSpan = styled.span`
  ...
  font-size: 0.9em;
`;
const LastSeenSpan = styled.span`
  color: #444444;
  font-size: 0.8em;
  margin: 0;
  padding: 0;
